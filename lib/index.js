// dsh-vditor-composer Host 半边
// 通过 webServer 注册 /plugins/vditor-composer 前缀路由：
...
const name = "dsh-vditor-composer";
...
async function writeUpload(scope, args) {
  ...
  const script = "$ErrorActionPreference='Stop'; $p = '" + filePath.replace(/'/g, "''") + "'; New-Item -ItemType Directory -Force -Path (Split-Path $p) | Out-Null; [IO.File]::WriteAllBytes($p, [Convert]::FromBase64String([Console]::In.ReadToEnd()))";
...
async function pickFiles(scope, args) {
  ...
  const ws = String(args.workspace || "");
  const cwd = ws || "C:\\";
  const script = "Add-Type -AssemblyName System.Windows.Forms; $d = New-Object System.Windows.Forms.OpenFileDialog; $d.Multiselect = $true; $d.Title = '选择要引用的文件'; $d.Filter = 'All files (*.*)|*.*'; if ($d.ShowDialog() -ne [System.Windows.Forms.DialogResult]::OK) { exit 0 }; $d.FileNames";
  const handle = subprocess.spawn({
    ...
    for (const entry of entries) {
      ...
      if (isDir) {
        if (q === "" || entryName.toLowerCase().includes(q)) {
          items.push({ path: relPath(rootAbs, dirAbs + "\\" + entryName).replace(/\\/g, "/"), name: entryName, kind: "dir" });
        }
        ...
        items.push({ path: relPath(rootAbs, dirAbs + "\\" + entryName).replace(/\\/g, "/"), name: entryName, kind: "file" });
      }
    }
  };
  try {
...
const PREFIX = "/plugins/vditor-composer";
