...
.dyn-vditor-wrap--hero {
  justify-content: center;
  padding-top: 24px;
  padding-bottom: 24px;
}
.dyn-vditor-card--workspace {
  cursor: pointer;
}
...
.dyn-vditor-card > .dyn-vditor-menu {
  top: 40px;
  left: 10px;
}
...
.dyn-vditor-plan-btn {
  ...
  border: none;
  border-radius: 999px;
  background: var(--dsw-alias-state-warn-tertiary, rgba(250, 204, 21, 0.15));
...
.dyn-vditor-plan-btn:hover:not(:disabled) { color: var(--dsw-alias-state-warn-primary, #d97706); }
.dyn-vditor-plan-btn:disabled { opacity: 0.6; cursor: default; }
.dyn-vditor-plan-x { display: inline-flex; align-items: center; }
.dyn-vditor-dock {
  display: flex;
  flex-direction: column;
  align-items: center;
...
.dyn-vditor-stats-line {
  color: var(--dsw-alias-label-tertiary, var(--dsw-alias-label-secondary));
  font-variant-numeric: tabular-nums;
  text-align: center;
}
.dyn-vditor-balance-line {
  display: inline-flex;
  align-items: center;
  gap: 6px;
  color: var(--dsw-alias-label-tertiary, var(--dsw-alias-label-secondary));
...
.dyn-vditor-balance-dot {
  ...
  height: 6px;
  border-radius: 50%;
  background: var(--dsw-alias-label-caption, #9ca3af);
  flex: none;
}
.dyn-vditor-balance-sep { opacity: 0.5; }
.dyn-vditor-balance-value { font-variant-numeric: tabular-nums; }
`
    ...
    const UserMarkdownNodeView = (props) => {
      const { node, renderMessageImages } = props
      ...
      React.useEffect(() => {
        if (!copied) return
        const tid = window.setTimeout(() => setCopied(false), 1500)
        return () => window.clearTimeout(tid)
      }, [copied])
      ...
      return React.createElement('div', { className: 'dyn-vditor-user-row', 'data-time-hover-root': true },
        images.length > 0 && typeof renderMessageImages === 'function' && renderMessageImages({ images, align: 'end' }),
    ...
    // 下拉菜单智能翻转：默认向下打开，下方空间不足且上方更充裕时改为向上
    const useMenuFlip = (open) => {
      ...
      React.useLayoutEffect(() => {
        ...
        if (spaceBelow < margin && spaceAbove > spaceBelow + margin) {
          setFlipUp(true)
        } else {
          setFlipUp(false)
        }
    ...
    const VditorComposer = (props) => {
      const { useSession, useSessions, useWorkspaces, useInput, inputActions, useProjection, sessionId, placeholder: ownerPlaceholder, variant, blocked, disabled: inert, accessory, onRequestWorkspace } = props
      ...
      const [cmdMenuRef, cmdFlipUp] = useMenuFlip(cmdOpen)
      const [permMenuRef, permFlipUp] = useMenuFlip(permOpen)
      const [modelMenuRef, modelFlipUp] = useMenuFlip(modelOpen)
      const [meterPanelRef, meterFlipUp] = useMenuFlip(meterOpen)
      ...
      const disabled = inert === true || removed || blocked !== undefined || running || noActions
      const modelLocked = removed || inert === true || noActions
      const workspaceTrigger = variant === 'hero' && inert === true && !removed && typeof onRequestWorkspace === 'function'
      ...
      const placeholder = (() => {
        ...
        if (blocked && blocked.reason) return blocked.reason
      ...
      React.useEffect(() => {
        if (variant !== 'composer' || sessionId === undefined) return
        let cancelled = false
        let controller = null
        const load = () => {
          if (controller !== null) controller.abort()
          controller = new AbortController()
          fetch('/dsh-balance/api/balance', { method: 'GET', headers: { accept: 'application/json' }, cache: 'no-store', signal: controller.signal })
            .then((r) => r.json())
            .then((value) => { if (!cancelled) setBalance(value && typeof value === 'object' ? value : { ok: false }) })
            .catch(() => { if (!cancelled) setBalance({ ok: false }) })
        }
        load()
        const timer = window.setInterval(load, 60000)
        return () => { cancelled = true; window.clearInterval(timer); if (controller !== null) controller.abort() }
      }, [variant, sessionId])
      ...
      React.useEffect(() => {
        ...
        const tid = window.setTimeout(() => setFileTip(null), 6000)
        return () => window.clearTimeout(tid)
      }, [fileTip])
      ...
      const optionLabel = (o) => (o && PERM_LABELS[o.value]) || (o && (o.name || o.label)) || displayName(o && o.value)
      const permLabel = (() => {
        const cur = permOptions.find((o) => o.value === permCurrent)
        return cur ? optionLabel(cur) : (permCurrent ? (PERM_LABELS[permCurrent] || displayName(permCurrent)) : '权限')
      })()
      ...
      const exitPlanMode = () => { runCommand('/plan off') }

      const statsGroups = []
      if (sessionStats && sessionStats.steps > 0) {
        statsGroups.push(sessionStats.turns + ' 轮 · ' + sessionStats.steps + ' 步')
        const durs = []
        if (sessionStats.llmMs > 0) durs.push('LLM ' + fmtDuration(sessionStats.llmMs))
        if (sessionStats.toolMs > 0) durs.push('工具 ' + fmtDuration(sessionStats.toolMs))
        if (durs.length > 0) statsGroups.push(durs.join(' · '))
        const speeds = []
        if (sessionStats.ttftSteps > 0) speeds.push('首字 ' + fmtDuration(sessionStats.ttftMs / sessionStats.ttftSteps))
        if (sessionStats.decodeMs > 0 && sessionStats.decodeTokens > 0) speeds.push(fmtTps(sessionStats.decodeTokens / (sessionStats.decodeMs / 1000)) + ' tok/s')
        if (speeds.length > 0) statsGroups.push(speeds.join(' · '))
      }
      if (tokenUsage) {
        const billedInput = (tokenUsage.uncachedInputTokens || 0) + (tokenUsage.cacheReadTokens || 0) + (tokenUsage.cacheWriteTokens || 0)
        const output = tokenUsage.outputTokens || 0
        if (billedInput > 0 || output > 0) statsGroups.push('输入 ' + fmtTokens(billedInput) + ' · 输出 ' + fmtTokens(output))
      }
      const statsLine = statsGroups.join(' | ')
      const balanceSummary = (() => {
        if (balance === null) return '查询中…'
        if (balance.ok !== true) return '暂时无法获取'
        const infos = Array.isArray(balance.balanceInfos) ? balance.balanceInfos : []
        if (infos.length === 0) return '暂无余额信息'
        return infos.map((info) => (info.currency ? info.currency + ' ' : '') + (typeof info.totalBalance === 'number' ? info.totalBalance.toFixed(2) : info.totalBalance)).join(' · ')
      })()
      const showDock = variant === 'composer' && sessionId !== undefined && (statsGroups.length > 0 || balance !== null)

      return React.createElement('div', { className: 'dyn-vditor-wrap' + (variant === 'hero' ? ' dyn-vditor-wrap--hero' : '') },
        ...
        React.createElement('div', {
          className: 'dyn-vditor-card' + (workspaceTrigger ? ' dyn-vditor-card--workspace' : ''),
          'data-composer-card': '',
          ...
          React.createElement('div', { className: 'dyn-vditor-toolbar-row' },
            React.createElement('div', { className: 'dyn-vditor-tools' },
              React.createElement('div', { className: 'dyn-vditor-drop' },
                ...
                cmdOpen && React.createElement('div', { ref: cmdMenuRef, className: 'dyn-vditor-menu' + (cmdFlipUp ? ' dyn-vditor-menu--up' : '') },
              ...
              permissions && React.createElement('div', { className: 'dyn-vditor-drop' },
                ...
                permOpen && React.createElement('div', { ref: permMenuRef, className: 'dyn-vditor-menu' + (permFlipUp ? ' dyn-vditor-menu--up' : '') },
              ...
              planActive && React.createElement('button', {
                type: 'button',
                className: 'dyn-vditor-plan-btn',
                title: 'plan mode 已开启 — 点击关闭（/plan off）',
                'aria-label': 'plan mode 已开启，按下关闭',
                disabled: disabled,
                onClick: () => exitPlanMode(),
              },
                React.createElement('span', { className: 'dyn-vditor-plan-label' }, 'Plan'),
                React.createElement('span', { className: 'dyn-vditor-plan-x', 'aria-hidden': true }, '×'),
              ),
              React.createElement('button', {
                type: 'button',
            ...
            React.createElement('div', { className: 'dyn-vditor-trailing' },
              usagePercent !== null && React.createElement('div', { className: 'dyn-vditor-drop' },
                ...
                meterOpen && React.createElement('div', { ref: meterPanelRef, className: 'dyn-vditor-meter-panel' + (meterFlipUp ? ' dyn-vditor-meter-panel--up' : '') },
              ...
              modelDirectories && React.createElement('div', { className: 'dyn-vditor-drop' },
                React.createElement('button', {
                  ...
                  title: '切换模型',
                  disabled: modelLocked || modelBusy || groups.length === 0,
                ...
                modelOpen && React.createElement('div', { ref: modelMenuRef, className: 'dyn-vditor-menu right' + (modelFlipUp ? ' dyn-vditor-menu--up' : '') },
        ...
        showDock && React.createElement('div', { className: 'dyn-vditor-dock' },
          statsLine !== '' && React.createElement('div', { className: 'dyn-vditor-stats-line' }, statsLine),
          balance !== null && React.createElement('div', { className: 'dyn-vditor-balance-line' },
            React.createElement('span', { className: 'dyn-vditor-balance-dot' }),
            React.createElement('span', { className: 'dyn-vditor-balance-label' }, 'DeepSeek 余额'),
            React.createElement('span', { className: 'dyn-vditor-balance-sep', 'aria-hidden': true }, '|'),
            React.createElement('span', { className: 'dyn-vditor-balance-value' }, balanceSummary),
          ),
        ),
      )
    }

    slots.inject('conversation.composer.bar', () => slots.register({
      name: 'conversation.composer.bar',
      priority: -10,
    }, VditorComposer))

    slots.inject('conversation.chat.node', function* () {
      yield slots.register({ name: 'conversation.chat.node', key: 'user', priority: -10 }, UserMarkdownNodeView)
      yield slots.register({ name: 'conversation.chat.node', key: 'steering', priority: -10 }, UserMarkdownNodeView)
    })
  }

  n.apply = apply
  n.inject = ['slots', 'modelDirectories', 'remote', 'remote.session']
  return t.exports
} });
