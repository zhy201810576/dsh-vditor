window.__ModuleLoader__.load({ id: `dsh-vditor-composer`, factory: (e) => {
  const t = { exports: {} }
  const n = t.exports
  const React = e('react')
  ...
  const apply = (ctx) => {
    ...
    const apiFetch = async (path, body) => {
      const res = await fetch('/plugins/vditor-composer' + path, body === undefined ? undefined : {
        method: 'POST',
    ...
    const STYLE_TAG = 'dsh-vditor-composer-style'
    const ensureStyles = () => {
...
.dyn-vditor-box .vditor {
  border: none;
  background: transparent;
  border-radius: 12px;
  overflow: hidden;
}
.dyn-vditor-box .vditor .vditor-toolbar {
  background: transparent;
  border-bottom: 1px solid var(--dsw-alias-border-l1);
  padding: 2px 4px;
}
.dyn-vditor-box .vditor .vditor-content,
.dyn-vditor-box .vditor .vditor-ir { background: transparent; }
.dyn-vditor-box .vditor .vditor-ir {
  color: var(--dsw-alias-label-primary);
  padding: 8px 4px;
}
.dyn-vditor-box .vditor .vditor-reset { color: var(--dsw-alias-label-primary); }
.dyn-vditor-box .vditor .vditor-ir .vditor-ir__node--expand { background: transparent; }
.dyn-vditor-box .vditor .vditor-content textarea::placeholder,
.dyn-vditor-box .vditor .vditor-ir textarea::placeholder {
  color: var(--dsw-alias-label-tertiary, var(--dsw-alias-label-secondary));
  opacity: 1;
}
...
.dyn-vditor-composer-menu-empty { padding: 8px 12px; font-size: 12px; color: var(--dsw-alias-label-secondary); }
...
`
      ...
      React.useEffect(() => {
        ...
        loadVditor().then(() => {
          ...
          try {
            ...
            console.error('[dsh-vditor-composer]', err)
    ...
    const VditorComposer = (props) => {
      ...
      React.useEffect(() => {
        ...
        try { dir.load().catch((err) => console.error('[dsh-vditor-composer]', err)) } catch (err) { console.error('[dsh-vditor-composer]', err) }
        return unsub
      ...
      const runCommand = async (line) => {
        ...
        } catch (err) { console.error('[dsh-vditor-composer]', err) }
      }
      ...
      const pickAtItem = (item) => {
        const replaced = lastLine.replace(/@([^\s@]*)$/, '@' + item.path)
      ...
      return React.createElement('div', { className: 'dyn-vditor-wrap' },
        ...
        React.createElement('div', { className: 'dyn-vditor-card', 'data-composer-card': '' },
          ...
          slashActive && React.createElement('div', { className: 'dyn-vditor-composer-menu' },
            slashCandidates.length === 0
              ? React.createElement('div', { className: 'dyn-vditor-composer-menu-empty' }, '没有匹配的命令')
          ...
          atActive && React.createElement('div', { className: 'dyn-vditor-composer-menu' },
            !cwd
              ? React.createElement('div', { className: 'dyn-vditor-composer-menu-empty' }, '未找到工作区路径')
              : atLoading
                ? React.createElement('div', { className: 'dyn-vditor-composer-menu-empty' }, '正在搜索工作区文件…')
                : atItems.length === 0
                  ? React.createElement('div', { className: 'dyn-vditor-composer-menu-empty' }, '未找到匹配文件')
                  : atItems.map((item, i) => React.createElement('button', {
                    ...
                    React.createElement('span', { className: 'dyn-vditor-menu-main' },
                      item.name,
                      React.createElement('span', { className: 'dyn-vditor-menu-desc' }, item.path),
                    ),
                  )),
          ),
